...
# To make each file self-contained, I copied them from http://incompleteideas.net/tiles/tiles3.py-remove
# with some naming convention changes (see http://incompleteideas.net/tiles/tiles3.html).
#
# Tile coding starts
class IHT:
...
class ValueFunction:
  ...
  # View the following webpage for more information http://incompleteideas.net/sutton/tiles/tiles3.html
...
def differential_semi_gradient_sarsa(value_function, max_steps):
  ...
  print(f"average_reward = {value_function.average_reward}")
...
def create_figure():
  ...
  plt.savefig("./server_access_control.png")
  plt.close()
