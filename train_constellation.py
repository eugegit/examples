# see the source: https://github.com/oxcsml/lie-transformer
...
from eqv_transformer.train_tools import (rotate, log_tensorboard, parse_reports, parse_reports_cpu, print_reports, load_checkpoint,
                                         save_checkpoint, ExponentialMovingAverage, param_count, get_component, get_average_norm)
import matplotlib.pyplot as plt
sys.path.append("/proj/examples")
import util
...
flags.DEFINE_string("data_config", "configs/constellation/constellation.py", "Path to a data config file.")
flags.DEFINE_string("model_config", "configs/constellation/eqv_transformer_model.py", "Path to a model config file.")
# Job management
...
# Logging
flags.DEFINE_integer("report_loss_every", 500, "Number of iterations between reporting minibatch loss.")
flags.DEFINE_integer("evaluate_every", 10000, "Number of iterations between reporting validation loss.")
flags.DEFINE_integer("save_check_points", 50, "frequency with which to save checkpoints, in number of epoches.")
flags.DEFINE_boolean("log_train_values", True, "Logs train values if True.")
flags.DEFINE_float("ema_alpha", 0.99, "Alpha coefficient for exponential moving average of train logs.")
...
def main():
  ...
  # Parse flags
  config = forge.config()
  config.__dict__["debug_group_config"] = None
  ...
  results_folder_name = osp.join(data_name, model_name, config.run_name, run_name)
  ...
  model_opt = torch.optim.Adam(model_params, lr=opt_learning_rate, betas=(config.beta1, config.beta2))
  ...
  train_iter = (start_epoch - 1) * (len(train_loader.dataset) // config.batch_size) + 1
  ...
  print(f"Number of model parameters: {num_params:,d}")
    ...
    for batch_idx, data in enumerate(train_loader):
      ...
      if config.log_train_values:
        ...
        if batch_idx % config.report_loss_every == 0:
          ...
          print_reports(reports, start_t, epoch, batch_idx, len(train_loader.dataset) // config.batch_size, prefix="train")

      if v:
        ax_loss.scatter(i_loss, outputs.reports["loss"].detach().cpu().numpy(), marker=".", color=util.my_color[0], label="loss" if i_loss==0 else "")
        ax_loss.scatter(i_loss, outputs.reports["acc"].detach().cpu().numpy(), marker=".", color=util.my_color[1], label="accuracy" if i_loss==0 else "")
        if i_loss == 0:
          ax_loss.legend(loc="upper right", ncol=1, scatterpoints=1, numpoints=1)
        i_loss += 1
        if i_loss % 100 == 0: fig_loss.canvas.flush_events()

      # Track stuff for debugging
      if config.debug:
        model.eval()
        with torch.no_grad():
          curr_params = { k: v.detach().clone() for k, v in model.state_dict().items() }
          ...
          if train_iter == 1:
            ...
            update_norms = []
            for (k_prev, v_prev), (k_curr, v_curr) in zip(prev_params.items(), curr_params.items()):
          ...
          #     log_tensorboard(summary_writer, train_iter, model_norms, "debug/avg_model_norms/")

          log_tensorboard(summary_writer, train_iter, { "avg_update_norm1": update_norm / num_params, "avg_grad_norm1": grad_norm / num_params}, "debug/")
          if batch_idx % 20 == 0:
            print(f"avg_update_norm = {update_norm / num_params :.2e}; avg_grad_norm1 = {grad_norm / num_params :.2e}")
      ...
      if batch_idx % config.evaluate_every == 0:
        ...
        with torch.no_grad():
          ...
          print_reports(reports, start_t, epoch, batch_idx, len(train_loader.dataset) // config.batch_size, prefix="test")
        model.train()
